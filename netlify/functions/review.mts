...
const REVIEW_SCHEMA = {
  ...
  properties: {
    ...
    verdict: { type: "string" },
    critical_issues: { type: "array", items: { $ref: "#/$defs/issue" } },
    ux_issues: { type: "array", items: { $ref: "#/$defs/issue" } },
    visual_issues: { type: "array", items: { $ref: "#/$defs/issue" } },
    business_opportunities: { type: "array", items: { type: "string" } },
    recommended_fixes: { type: "array", items: { $ref: "#/$defs/fix" } },
...
function buildPrompt({ context, task, screen, focus, mode }) {
...
async function imageFromUrl(url) {
  ...
  if (contentLength > MAX_IMAGE_BYTES) {
    throw new Error("Screenshot is larger than 15 MB.");
  }
  ...
  if (!mime.startsWith("image/")) {
    throw new Error("The supplied Figma URL did not return an image.");
  }
...
function getReviewInputs(params) {
  return {
...
export default async (req) => {
  if (req.method === "GET") {
    ...
    if (!imageUrl) {
      return json({ ok: false, error: "Missing image_url. Pass a Figma screenshot URL." }, 400);
    }
    ...
    try {
      ...
      return json(
        { ok: false, error: error instanceof Error ? error.message : "Unknown reviewer error." },
        500,
      );
    }
  }
